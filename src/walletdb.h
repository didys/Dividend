...
// Copyright (c) 2016-2018 The Dividend developers
...
public:
    ...
    bool WriteZDVDSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZDVDSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZDVDSeed_deprecated(uint256& seed);
    bool EraseZDVDSeed();
    bool EraseZDVDSeed_deprecated();

    bool WriteZDVDCount(const uint32_t& nCount);
    bool ReadZDVDCount(uint32_t& nCount);
