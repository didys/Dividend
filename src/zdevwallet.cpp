...
CzDVDWallet::CzDVDWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZDVDSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDVDSeed_deprecated()) {
                    LogPrintf("%s: Updated zDVD seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzDVDWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDVDCount(nCountLastUsed);
    else if (!walletdb.ReadZDVDCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDVDWallet::Lock()
{
    seedMaster = 0;
}

void CzDVDWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDVDWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDVD(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDVDWallet::LoadMintPoolFromDB()
{
...
void CzDVDWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDVDWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDVDWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zdevTracker->HasPubcoinHash(pMint.first)) {
...
bool CzDVDWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDVD(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    pwalletMain->zdevTracker->Add(dMint, true);
...
void CzDVDWallet::SeedToZDVD(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDVDWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDVDWallet::GenerateDeterministicZDVD(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDVDWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDVDWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
