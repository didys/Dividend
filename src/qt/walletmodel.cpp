...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered dividend address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 DVD.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal dividend:URI (dividend:XyZ...?message=example)
            {
...
static void NotifyzDVDReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzDVDReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzDVDReset.connect(boost::bind(NotifyzDVDReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzDVDReset.disconnect(boost::bind(NotifyzDVDReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zdevTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
