...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::DVD);
    ...
    if (!settings.contains("nAnonymizeDividendAmount"))
        settings.setValue("nAnonymizeDividendAmount", 1000);

    nAnonymizeDividendAmount = settings.value("nAnonymizeDividendAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeDividendAmount"))
        SoftSetArg("-anonymizedividendamount", settings.value("nAnonymizeDividendAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in dividend.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDividendAmount:
            return QVariant(nAnonymizeDividendAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDividendAmount:
            nAnonymizeDividendAmount = value.toInt();
            settings.setValue("nAnonymizeDividendAmount", nAnonymizeDividendAmount);
            emit anonymizeDividendAmountChanged(nAnonymizeDividendAmount);
            break;
        case CoinControlFeatures:
