...
#ifndef DIVIDEND_ZDVDWALLET_H
#define DIVIDEND_ZDVDWALLET_H
...
public:
    CzDVDWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZDVD(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZDVD(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //DIVIDEND_ZDVDWALLET_H
