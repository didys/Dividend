...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT25000IFMN = 3,
    ONLY_NONDENOMINATED_NOT25000IFMN = 4, // ONLY_NONDENOMINATED and not 25000 DVD at the same time
    ONLY_25000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zDVD send
enum ZerocoinSpendStatus {
    ZDVD_SPEND_OKAY = 0,                            // No error
    ZDVD_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZDVD_WALLET_LOCKED = 2,                         // Wallet was locked
    ZDVD_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZDVD_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZDVD_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZDVD_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZDVD_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZDVD_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZDVD_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZDVD_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZDVD_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZDVD_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZDVD_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZDVD_SPENT_USED_ZDVD = 14,                      // Coin has already been spend
    ZDVD_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZDVD_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZDevBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZDVDOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzdevAuto) const;
    ...
    */
    ...
    CzDVDWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzDVDTracker> zdevTracker;
    ...
    void setZWallet(CzDVDWallet* zwallet)
    {
        zwalletMain = zwallet;
        zdevTracker = std::unique_ptr<CzDVDTracker>(new CzDVDTracker(strWalletFile));
    }

    CzDVDWallet* getZWallet() { return zwalletMain; }
    ...
    void setZDevAutoBackups(bool fEnabled)
    {
    ...
    /** zDVD reset */
    boost::signals2::signal<void()> NotifyzDVDReset;
