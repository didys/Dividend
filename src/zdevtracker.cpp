...
CzDVDTracker::CzDVDTracker(std::string strWalletFile)
{
...
CzDVDTracker::~CzDVDTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzDVDTracker::Init()
{
...
bool CzDVDTracker::Archive(CMintMeta& meta)
{
...
bool CzDVDTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzDVDTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzDVDTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzDVDTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzDVDTracker::GetSerialHashes()
{
...
CAmount CzDVDTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzDVDTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzDVDTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzDVDTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzDVDTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzDVDTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzDVDTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzDVDTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzDVDTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzDVDTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzDVDTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDVDTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDVDTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzDVDTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzDVDTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzDVDTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzDVDTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzdev from DB\n", __func__, listDeterministicDB.size());
    }
